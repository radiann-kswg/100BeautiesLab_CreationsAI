...
- **デスクトップ版 Claude（Cowork モード）**: Stop フックは発火しないため、追従待ちを確認するには `bash tools/check-creations-db-update.sh` を手動実行する（終了コード `10`=追従済み / `0`=更新あり / `1`=エラー）。
...
- デスクトップ版 Cowork で追従待ちを確認するには `bash tools/check-creations-db-update.sh` を手動実行する。終了コード `10`=追従済み / `0`=更新あり / `1`=エラー。
